...
angular.module("tinyurlApp")
    .controller("urlController", ["$scope", "$http", "$routeParams", function ($scope, $http, $routeParams) {
        $http.get("/api/v1/urls/" + $routeParams.shortUrl) // get shortUrl from .when("/urls/:shortUrl" in app.js
            .success(function (data) {
                $scope.longUrl = data.longUrl;
                $scope.shortUrl = data.shortUrl;
                $scope.shortUrlToShow = "localhost:3000/" + data.shortUrl
            });
        ...
        // Initialize so because html can only pass variables, not strings
        $scope.hour = "hour";
        $scope.day = "day";
        $scope.month = "month";
        $scope.time = $scope.hour; // Set "hour" as default
